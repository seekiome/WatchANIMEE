...
function getRoom(id) {
  if (!rooms[id]) rooms[id] = {
    hostClientId: null, hostWs: null, clients: new Map(),
    state: { playing: false, time: 0, updatedAt: Date.now() },
    hasVideo: false, videoFile: null, videoOrigName: null,
    queue: [], queueIdx: -1, createdAt: Date.now(), lastActivity: Date.now()
  };
  return rooms[id];
}
...
function broadcast(roomId, data, exceptWs = null) {
  ...
  if (room.hostWs && room.hostWs !== exceptWs && room.hostWs.readyState === 1) room.hostWs.send(msg);
  room.clients.forEach(ws => { if (ws !== exceptWs && ws.readyState === 1) ws.send(msg); });
}

function sendTo(ws, data) { if (ws && ws.readyState === 1) ws.send(JSON.stringify(data)); }
function getViewerCount(room) { return room.clients.size + (room.hostWs ? 1 : 0); }
function getCurrentTime(room) { return room.state.playing ? room.state.time + (Date.now() - room.state.updatedAt) / 1000 : room.state.time; }
...
function safeDelete(filename, delay = 30000) {
  setTimeout(() => {
    if (!getFilesInUse().has(filename)) { try { fs.unlinkSync(path.join(UPLOAD_DIR, filename)); } catch {} }
  }, delay);
}
...
function playQueueItem(roomId, idx) {
  ...
  const item = room.queue[idx]; if (!item.serverFile) return;
  room.queueIdx = idx;
  room.videoFile = item.serverFile; room.videoOrigName = item.origName;
  room.hasVideo = true; room.state = { playing: false, time: 0, updatedAt: Date.now() };
  room.lastActivity = Date.now();
  broadcast(roomId, { type: 'video_ready', streamUrl: `/video-stream/${roomId}`, origName: item.origName, serverTime: Date.now(), state: { playing: false, time: 0 }, queueIdx: idx, queueLen: room.queue.length });
}

function broadcastQueue(roomId) {
  const room = rooms[roomId]; if (!room) return;
  broadcast(roomId, { type: 'queue_update', queue: room.queue.map((q, i) => ({ id: q.id, origName: q.origName, idx: i })), queueIdx: room.queueIdx });
}
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'join') {
      roomId = msg.roomId; clientId = msg.clientId || crypto.randomBytes(4).toString('hex');
      if (!roomId || typeof roomId !== 'string' || !/^[A-Z0-9]{4,10}$/.test(roomId)) { sendTo(ws, { type: 'error', message: 'Invalid room ID' }); return; }
      ...
      if (isReconnectingHost || (msg.isHost && !room.hostClientId)) {
        isHost = true; room.hostClientId = clientId; room.hostWs = ws;
      } else { isHost = false; room.clients.set(clientId, ws); }
      const currentTime = getCurrentTime(room);
      sendTo(ws, { type: 'init', hasVideo: room.hasVideo, videoOrigName: room.videoOrigName, serverTime: Date.now(), state: { ...room.state, time: currentTime }, viewers: getViewerCount(room), clientId, isHost, queueIdx: room.queueIdx, queueLen: room.queue.length });
      if (room.hasVideo && room.videoFile) sendTo(ws, { type: 'video_ready', streamUrl: `/video-stream/${roomId}`, origName: room.videoOrigName, serverTime: Date.now(), state: { ...room.state, time: currentTime }, queueIdx: room.queueIdx, queueLen: room.queue.length });
      if (room.queue.length > 0) sendTo(ws, { type: 'queue_update', queue: room.queue.map((q, i) => ({ id: q.id, origName: q.origName, idx: i })), queueIdx: room.queueIdx });
      broadcast(roomId, { type: 'viewers', count: getViewerCount(room) }, ws);
    }

    if (msg.type === 'sync' && isHost) {
      const room = rooms[roomId]; if (!room) return;
      room.state = { playing: msg.playing, time: msg.time, updatedAt: Date.now() }; room.lastActivity = Date.now();
    ...
    if (msg.type === 'video_ready' && isHost) {
      ...
      if (serverFile) { room.videoFile = serverFile; room.videoOrigName = msg.origName || room.videoOrigName; room.hasVideo = true; room.lastActivity = Date.now(); }
      broadcast(roomId, { type: 'video_ready', streamUrl: `/video-stream/${roomId}`, origName: room.videoOrigName, serverTime: Date.now(), state: { playing: false, time: 0 } }, ws);
    }
    ...
    if (msg.type === 'queue_play' && isHost) { playQueueItem(roomId, msg.idx); }

    if (msg.type === 'queue_remove' && isHost) {
      const room = rooms[roomId]; if (!room) return;
      const idx = room.queue.findIndex(q => q.id === msg.id); if (idx === -1) return;
    ...
    if (msg.type === 'queue_reorder' && isHost) {
      ...
      const [moved] = room.queue.splice(fromIdx, 1); room.queue.splice(toIdx, 0, moved);
  ...
  ws.on('close', () => {
    ...
    if (isHost) { room.hostWs = null; broadcast(roomId, { type: 'host_left' }); }
    else { room.clients.delete(clientId); }
    broadcast(roomId, { type: 'viewers', count: getViewerCount(room) });
    if (!room.hostWs && room.clients.size === 0 && !room.hostClientId) deleteRoom(roomId);
  });
});
...
const upload = multer({ storage, limits: { fileSize: 4 * 1024 * 1024 * 1024 }, fileFilter: (req, file, cb) => { if (file.mimetype.startsWith('video/')) cb(null, true); else cb(new Error('Only video files allowed')); } });

app.post('/upload/:roomId', (req, res) => {
  const roomId = req.params.roomId;
  if (!roomId || !/^[A-Z0-9]{4,10}$/i.test(roomId)) return res.status(400).json({ error: 'Invalid room ID' });
  upload.single('video')(req, res, (err) => {
    if (err instanceof multer.MulterError) { if (err.code === 'LIMIT_FILE_SIZE') return res.status(413).json({ error: 'File too large (max 4GB)' }); return res.status(400).json({ error: err.message }); }
    if (err) return res.status(400).json({ error: err.message });
    if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
    const r = getRoom(roomId); r.lastActivity = Date.now();
    r.videoFile = req.file.filename; r.videoOrigName = req.file.originalname; r.hasVideo = true;
    res.json({ origName: req.file.originalname, serverFile: req.file.filename, streamUrl: `/video-stream/${roomId}` });
  });
});
...
function getContentType(filename) {
  ...
  const types = { mp4: 'video/mp4', mkv: 'video/x-matroska', avi: 'video/x-msvideo', mov: 'video/quicktime', webm: 'video/webm', m4v: 'video/mp4', mpeg: 'video/mpeg', mpg: 'video/mpeg', ogv: 'video/ogg' };
...
// ── HEALTH ──
app.get('/health', (req, res) => res.json({ status: 'ok', uptime: Math.floor(process.uptime()), rooms: Object.keys(rooms).length }));
...
setInterval(() => {
  ...
  for (const [roomId, room] of Object.entries(rooms)) if (room.lastActivity < cutoff) deleteRoom(roomId);
}, 30 * 60 * 1000);
